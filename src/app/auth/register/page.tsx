...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.agreeTerms) {
      setError("Необходимо принять оферту");
      return;
    }

    // TODO: integrate with API
    setError("Регистрация будет доступна после подключения базы данных");
  };
    ...
    <div className="pt-24 min-h-screen flex items-center justify-center px-4">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-xs text-white/40 mb-2">Логин</label>
          ...
          <label className="flex items-start gap-3 cursor-pointer">
            <div className="mt-0.5">
              <CheckSquare
                size={18}
                className={
                  formData.agreeTerms ? "text-rise-red" : "text-white/20"
                }
                onClick={() =>
                  setFormData({
                    ...formData,
            ...
            <span className="text-xs text-white/40 leading-relaxed">
              Я принимаю{" "}
              <span className="text-rise-red cursor-pointer hover:underline">
                оферту/договор об оказании услуг
              </span>{" "}
        ...
        <p className="text-center text-sm text-white/30 mt-6">
          Уже есть аккаунт?{" "}
          <Link href="/auth/login" className="text-rise-red hover:underline">
            Войти
          </Link>
        </p>
      </motion.div>
